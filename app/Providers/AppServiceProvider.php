...
class AppServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        ...
        if (! $this->app->runningInConsole()) {
            ...
            // SHARE TO ALL ROUTES
            $bedroomdistinct  = Property::select('kamar_tidur')->distinct()->get();
            view()->share('bedroomdistinct', $bedroomdistinct);

            $cities   = Property::select('kota')->distinct()->get();
            $citylist = array();
            foreach($cities as $city){
                $citylist[$city['kota']] = NULL;
            }
            ...
            view()->composer('pages.search', function($view) {
                $view->with('bathroomdistinct', Property::select('kamar_tidur')->distinct()->get());
            });
