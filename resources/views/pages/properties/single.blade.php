            ...
            <div class="row">
                <div class="col s12 m8">
                    <div class="single-title">
                        <h4 class="single-title">{{ $property->nama_rumah }}</h4>
                    </div>

                    <div class="address m-b-30">
                        <i class="small material-icons left">place</i>
                        <span class="font-18">{{ $property->alamat }}</span>
                    </div>
                    ...
                    <div>
                        ...
                        <span class="btn btn-small disabled b-r-20">Kamar Tidur : {{ $property->kamar_tidur}} </span>
                        <span class="btn btn-small disabled b-r-20">Kamar Mandi : {{ $property->kamar_mandi}} </span>
                        <span class="btn btn-small disabled b-r-20">Jumlah Lantai : {{ $property->jumlah_lantai}} </span>
                    </div>
                </div>
                <div class="col s12 m4">
                    <div>
                        <h4 class="left">${{ $property->harga_rumah }}</h4>
                        <button type="button" class="btn btn-small m-t-25 right disabled b-r-20"> Di {{ $property->kategori }}</button>
                        <br>
                        <button type="button" class="btn btn-small m-t-25 right disabled b-r-20"> Desain Rumah : {{ $property->desain }}</button>
                    </div>
                </div>
            </div>
            <div class="row">
                ...
                <div class="col s12 m8">
                    ...
                    @else
                        <div class="single-image">
                            @if(Storage::disk('public')->exists('property/'.$property->image) && $property->image)
                                <img src="{{Storage::url('property/'.$property->image)}}" alt="{{$property->nama_rumah}}" class="imgresponsive">
                            @endif
                        </div>
                    @endif
                        ...
                        @if($property->features)
                            <ul class="collection with-header">
                                ...
                                @foreach($property->features as $feature)
                                    <li class="collection-item">{{$feature->nama}}</li>
                                @endforeach
                            </ul>
                        @endif
                    </div>
                    ...
                    <div class="card-no-box-shadow card">
                        <div class="p-15 grey lighten-4">
                            <h5 class="m-0">Lokasi Strategis</h5>
                        </div>
                        <div class="single-narebay p-15">
                            {!! $property->lokasi_strategis !!}
                        </div>
                    </div>
                            ...
                            @foreach($property->comments as $comment)
                                    ...
                                    <div class="comment">
                                        ...
                                        <div class="content">
                                            <div class="author-name">
                                                <strong>{{ $comment->users->nama }}</strong>
                                ...
                                @foreach($comment->children as $commentchildren)
                                    <div class="comment children">
                                        ...
                                        <div class="content">
                                            <div class="author-name">
                                                <strong>{{ $commentchildren->users->nama }}</strong>
                ...
                <div class="col s12 m4">
                    <div class="clearfix">
                            ...
                            <ul class="collection with-header m-t-0">
                                        ...
                                        <div class="card horizontal card-no-shadow">
                                            ...
                                            <div class="card-stacked">
                                                <div class="p-l-10 p-r-10">
                                                    <h5 class="m-t-b-0">{{ $property->user->nama }}</h5>
                                                    <strong>{{ $property->user->email }}</strong>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="p-l-10 p-r-10">
                                            <p>{{ $property->user->deskripsi }}</p>
                                ...
                                <li class="collection agent-message">
                                    <form class="agent-message-box" action="" method="POST">
                                        ...
                                        <div class="box">
                                            <input type="number" name="phone" placeholder="Ketik Phone Kamau...">
                                        </div>
                                        <div class="box">
                                        ...
                                        <div class="box">
                                            <button id="msgsubmitbtn" class="btn waves-effect waves-light w100 indigo" type="submit">
                                                SEND
                                                <i class="material-icons left">KIRIM</i>
                                            </button>
                                        </div>
                                    </form>
                                </li>
                        ...
                        <div>
                            <ul class="collection with-header">
                                <li class="collection-header grey lighten-4">
                                    <h5 class="m-0">DAFTAR KOTA</h5>
                                </li>
                                @foreach($cities as $kota)
                                    <li class="collection-item p-0">
                                        <a class="city-list" href="{{ route('property.city',$kota->kota_slug) }}">
                                            <span>{{ $kota->kota }}</span>
                                        </a>
                                    </li>
                                @endforeach
